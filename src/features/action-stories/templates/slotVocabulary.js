...
export const SLOT_VOCABULARY = Object.freeze({
  ...
  calendar: { binding: 'proposal.calendar', blockType: 'calendarGantt', tier: 'conditional', note: 'Named lanes with events positioned on a shared week grid (S9.19\'s own promo calendar). Kept below the 20% corpus floor deliberately, same reasoning as bridge/matrix above: wires the newly-registered CalendarGanttBlock rather than leaving S9.19\'s real lane/week data flattened into a generic itemQueue.' },
  ...
  basis: { binding: 'proposal.basis', blockType: 'labelValueList', tier: 'conditional', note: '6/6. The provenance figures behind the confidence. Fills the `provenance` section, which the audit found declared and permanently empty.' },

  alternatives: { binding: 'proposal.alternatives', blockType: 'table', tier: 'conditional', note: 'The scenarios the operator may switch between ("Conservative / Balanced / Aggressive"). Reference `slates`/`modes`/`offerModes`/`slateTabs`/`caps`.' },
  item_groups: { binding: 'proposal.item_groups', blockType: 'itemQueue', tier: 'conditional', note: 'The selectable GROUPS of items, each with its own revenue/margin/capital effect. Reference `groups`.' },
  ...
  next_actions: { binding: 'proposal.next_actions', blockType: 'itemQueue', tier: 'conditional', note: 'The routes out of this decision, each with the reference\'s own explanation of what it does. Reference `actions`/`routes`.' },
  totals_rows: { binding: 'totals.rows', blockType: 'labelValueList', tier: 'conditional', note: 'The decision\'s roll-up totals. Reference `totals`/`rollup`/`summary`/`liveStats`.' },
  ...
  guardrail_checks: { binding: 'guardrails.checks', blockType: 'labelValueList', tier: 'conditional', note: 'The governance checklist. Reference `checks`/`guardParts` — rows identified by `label`, `name` OR `text`.' },
  ...
  progress_rows: { binding: 'execution.progress_rows', blockType: 'table', tier: 'conditional', note: 'Per-target execution state. Corpus `progressRows` (14 uses).' },
