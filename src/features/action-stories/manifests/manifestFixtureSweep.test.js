...
const manifestModules = import.meta.glob('./*.json', { eager: true });
const fixtureModules = import.meta.glob('/src/features/action-stories/data/raw/*/*.json', { eager: true });

function fixtureFor(code, stageKey) {
  const path = `/src/features/action-stories/data/raw/${code}/${stageKey}.json`;
...
describe('manifest + fixture sweep — zero orphaned blocks across every real workflow', () => {
  ...
  describe.each(manifestsByCode)('$code', ({ code, stageManifests }) => {
    it.each(stageManifests.map((m) => [m.stageKey, m]))('%s: every block resolves and validates cleanly', (stageKey, manifest) => {
      ...
      expect(fixture, `no fixture found at data/raw/${code}/${stageKey}.json`).toBeTruthy();
